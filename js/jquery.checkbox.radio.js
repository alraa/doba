...
$(document).ready(function() {
	...
	var checkbox_w = 17; // checkbox width
	var checkbox_h = 17; // checkbox height
	var radio_w = 44; // radio width
	var radio_h = 39; // radio height
	...
	$('input[type=checkbox]').each(function() {
		...
		if($(this).is(':checked') && $(this).is(':disabled')) {
			...
			custom_checkbox.css({'background-position' : '0 -' + checkbox_h + 'px', 'cursor' : 'pointer'});
	...
	$('input[type=radio]').each(function() {
		...
		if($(this).is(':checked') && $(this).is(':disabled')) {
			...
			custom_radio.css({'background-position' : '0 -' + radio_h + 'px', 'cursor' : 'pointer'});
			
		} else if($(this).is(':disabled')) {
			custom_radio.css({'background-position' : '0 -' + radio_h * 2 + 'px', 'cursor' : 'default'});
		} else {
			custom_radio.css({'background-position' : '0 0', 'cursor' : 'pointer'});
		}
	...
	function changeRadio(el) {
		...
		$('input[type=radio][name=' + radio.attr('name') + ']').each(function() {
			if(!$(this).is(':disabled')) {
				...
				custom_radio.parents('.b-filter__radio-group').find('.radio-label').removeClass('radio-label-active');
				custom_radio.parent().addClass('radio-label-active');
			} else if($(this).is(':checked')) {
